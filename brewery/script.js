...
function createBrewerylist(element){
    ...
    <div class="explain">
    <p id="type">Brewery Type: ${element.brewery_type}</p>
    <p id="address">Address: ${element.address_2}</p>
    <p class="author com">Author: ${element.authors}</p>
    <p id="urls">Website Url: ${element.website_url}</p>
    <p id="phone">Contact No: ${element.phone}</p>`
...
async function createContainer () {
    ...
    if(inp){
        ...
        let inputUrl = await fetch(`https://api.openbrewerydb.org/breweries?by_city=${inp}&per_page=3`);
